...
const VocabularyListTopic = () => {
  ...
  if (loading) return <LoadingSpinner message="Đang tải danh sách chủ đề từ vựng..." />;
  if (error) return <ErrorMessage error={error} onRetry={fetchMainTopics} />;

  const breadcrumbItems = [
    { label: 'Trang chủ', path: '/' },
    { label: 'Từ vựng theo chủ đề', path: '/vocabulary' },
  ];

  return (
    <VocabularyLayout
      title="Từ vựng theo chủ đề"
      description="Khám phá và luyện tập với hàng nghìn từ vựng được tổ chức theo chủ đề. Nâng cao vốn từ vựng một cách có hệ thống và hiệu quả."
    ...
    >
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center bg-white/90 rounded-lg border border-gray-200 p-1">
                ...
                >
                  ...
                  <option value="testCount">Sắp xếp theo số chủ đề con</option>
                </select>
              </div>
        ...
        <EmptyState
          ...
          description={
            filters.searchTerm
              ...
              : 'Hiện tại chưa có chủ đề từ vựng nào được tạo.'
          }
          action={
            filters.searchTerm && (
              <button
      ...
      {/* Global Vocabulary Topic Modal */}
      <VocabularyTopicModal 
        isOpen={!!modalTopic} 
